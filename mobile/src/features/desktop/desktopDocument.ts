...
import { desktopInputRuntime } from './desktopInputRuntime';
import { desktopPeerRuntime } from './desktopPeerRuntime';
...
<body>
  ...
  <script>
    ...
    const flushViewerState = () => {
      viewerStateTimer = null;
      postBridgeMessage({ type: 'desktop-state', state: viewerState });
    };

    const updateViewerState = (patch) => {
      Object.assign(viewerState, patch);
      if (!viewerStateTimer) {
        viewerStateTimer = window.setTimeout(flushViewerState, 0);
      }
    };

    const setStatus = (status, statusIsError = false) => {
      updateViewerState({ status, statusIsError: Boolean(statusIsError) });
    };
    ...
    const buildScreensState = () => {
      ...
      updateViewerState({ screens, selectedScreenId });
    };
...
${desktopInputRuntime}
    ...
    window.latitudeMobileCommand = handleDesktopCommand;
